...
void read_preferences()
{
  ...
  if (g_key_file_load_from_file(rc_key, rc_file, G_KEY_FILE_NONE, NULL))
  {
    ...
    if ((!prefs.history_limit) || (prefs.history_limit > DEF_HISTORY_LIMIT) || (prefs.history_limit < 0))
      prefs.history_limit = DEF_HISTORY_LIMIT;
    if ((!prefs.items_menu) || (prefs.items_menu > DEF_ITEMS_MENU) || (prefs.items_menu < 0))
